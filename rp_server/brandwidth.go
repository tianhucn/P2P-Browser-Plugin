...
func (p *PingOption) ping3(host string, args map[string]interface{}) (aveTime int, loseRatio int) {
	//要发送的回显请求数
	var count int = 4
	//要发送缓冲区大小,单位：字节
	var size int = 16
	...
	conn, _ := net.DialTimeout("ip4:icmp", host, time.Duration(timeout*1000*1000))
	...
	for count > 0 || neverstop {
		...
		conn, _ = net.DialTimeout("ip:icmp", host, time.Duration(timeout*1000*1000))
	...
	aveTime = sumT / sendN
	loseRatio = int(lostN * 100 / sendN)
	stat3(host, sendN, lostN, recvN, shortT, longT, sumT)
	return aveTime, loseRatio
	//todo 先注释，用下一行测试
